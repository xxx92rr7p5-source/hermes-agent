...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    if sys.platform == "win32":
        # 1+4 + upstream 兼容: 同时设 CREATE_NO_WINDOW (隐藏 console 弹窗) + CREATE_NEW_PROCESS_GROUP + DETACHED_PROCESS
        # upstream windows_detach_flags() 默认包含后者两项但缺 CREATE_NO_WINDOW (流云 P204-P205 修复)
...
async def open_profile_terminal_endpoint(name: str):
    try:
        ...
        if sys.platform.startswith("win"):
            # GUI 后端 spawn profile setup terminal: 必须隐藏 cmd 窗口,
            # 否则 Windows 会弹一闪而过的 cmd 窗口 (用户硬偏好: 流云).
