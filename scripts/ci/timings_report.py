...
def collect_timings(token: str, repo: str, run_id: str, head_sha: str) -> dict:
    ...
    orch_jobs = api_get(f"/repos/{owner}/{repo_name}/actions/runs/{run_id}/jobs", token)
    ...
    for sr in sub_runs:
        sr_id = sr["id"]
        sr_name = sr.get("name", "")
        sr_jobs = api_get(f"/repos/{owner}/{repo_name}/actions/runs/{sr_id}/jobs", token)
        for j in sr_jobs:
...
def generate_summary(timings: dict, baseline: dict | None = None) -> str:
    ...
    # Per-job table (only jobs with baseline for diff, all for current-only)
    lines.append("### Per-Job Comparison\n")
    lines.append("| Job | Current | Baseline | Delta |")
    lines.append("|-----|---------|----------|-------|")
    for j in timings.get("jobs", []):
        dur = j.get("duration_s")
        bl = bl_map.get(j["name"])
        bl_dur = bl.get("duration_s") if bl else None
        delta_text, _ = fmt_delta(dur, bl_dur)
        lines.append(f"| {j['name']} | {fmt_dur(dur)} | {fmt_dur(bl_dur)} | {delta_text} |")
    lines.append("")

    lines.append(
        "Full interactive report with gantt chart and per-step breakdown: "
        "download the `ci-timings-report` artifact from this workflow run.\n"
    )
...
def main():
    ...
    if args.from_json:
        ...
        missing = [k for k, v in [("GITHUB_TOKEN", token), ("GITHUB_REPOSITORY", repo),
                                   ("GITHUB_RUN_ID", run_id), ("GITHUB_SHA", head_sha)] if not v]
        if missing:
            print(f"::error::Missing environment variables: {', '.join(missing)}", file=sys.stderr)
            print("Set them or use --from-json to load from a file.", file=sys.stderr)
            sys.exit(1)

        try:
            timings = collect_timings(token, repo, run_id, head_sha)
        except Exception as e:
            print(f"::error::Failed to collect timings: {e}", file=sys.stderr)
            sys.exit(1)
