...
import { parseWindowDeepLink } from './session-windows'

describe('parseWindowDeepLink (1+4 deep-link handoff)', () => {
  it('parses hermes://window/session/<id>', () => {
    expect(parseWindowDeepLink('hermes://window/session/abc-123')).toEqual({
      kind: 'session',
      sessionId: 'abc-123',
      watch: false,
    })
  ...
  it('parses hermes://window/session/<id>?watch=1', () => {
    expect(parseWindowDeepLink('hermes://window/session/abc-123?watch=1')).toEqual({
      kind: 'session',
      sessionId: 'abc-123',
      watch: true,
    })
  ...
  it('parses hermes://window/new', () => {
    expect(parseWindowDeepLink('hermes://window/new')).toEqual({ kind: 'new' })
  })

  it('returns null kind for non-window URLs', () => {
    expect(parseWindowDeepLink('hermes://blueprint/foo')).toEqual({ kind: null })
    expect(parseWindowDeepLink('https://example.com')).toEqual({ kind: null })
  })

  it('URL-decodes session id with special chars', () => {
    expect(parseWindowDeepLink('hermes://window/session/a%20b%2Fc')).toEqual({
      kind: 'session',
      sessionId: 'a b/c',
      watch: false,
    })
  ...
  it('handles empty/malformed input', () => {
    expect(parseWindowDeepLink('')).toEqual({ kind: null })
    expect(parseWindowDeepLink(null as any)).toEqual({ kind: null })
    expect(parseWindowDeepLink('not-a-url')).toEqual({ kind: null })
  })
})
